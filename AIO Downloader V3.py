...
def run_ffmpeg_with_progress(cmd, total_secs):
    """
    Audio conversion progress bar (tqdm) based on ffmpeg out_time_ms.
    """
...
def process_and_embed_image(mp3_path, img_url, hdrs, cookie=None, keep_original=False):
    """
    Download image → Prefer embedding in its original format (JPEG/PNG) → Embed into MP3.
    - Keeps ID3v2.3
    - Sets APIC description to blank
    - Avoids front-cover classification (PictureType remains OTHER)
    """
    tmp_img = None
    tmp_conv = None
    ...
    # Prepare image: keep JPEG/PNG; convert others to PNG
    print("Preparing image for embedding...")
    is_png = src_ext == ".png"
    ...
    if not (is_png or is_jpg):
        try:
            ...
            if tmp_conv and os.path.exists(tmp_conv):
                try: os.remove(tmp_conv)
                except OSError: pass
            return None
    else:
        embed_path = tmp_img
        codec = "mjpeg" if is_jpg else "png"  # match source format

    # Embed cover (blank APIC description, no front-cover hints)
    print("Embedding cover into MP3...")
    try:
        subprocess.run([
            "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
            ...
            "-id3v2_version", "3",
            "-metadata:s:v", "title=",  # blank APIC description
            out_path
        ], check=True)
    except subprocess.CalledProcessError as e:
        print(f"ERROR: ffmpeg failed to embed cover art: {e}")
        return None
    finally:
...
def run_download(base_dir, embed_cover, keep_original=False):
    ...
    if not ok:
        print("ERROR: Download failed (token may be expired). Paste a fresh cURL and try again.")
        return
    ...
    if embed_cover == "y":
        ...
        if img_url:
            ...
            if new_path:
                print(f"Cover embedded: {new_path}")
            else:
                print("ERROR: Embedding cover failed.")
        else:
            print("Skipping cover embedding.")
